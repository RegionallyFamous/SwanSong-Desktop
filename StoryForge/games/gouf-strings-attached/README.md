...
```sh
python3 scripts/build_wscvn_game.py gouf-strings-attached
```
...
Run both compiled branches through the installed SwanSong engine with:

```sh
python3 scripts/playtest_wscvn_swansong.py gouf-strings-attached
```

The trace is written to `reports/swansong-playthrough-report.json` and ending
captures to `assets/swansong-playthrough/`.
