...
| --- | --- | --- |
| Story Forge novel framework | [`StoryForge/`](https://github.com/RegionallyFamous/SwanSong-Desktop/tree/main/StoryForge) in `RegionallyFamous/SwanSong-Desktop` | Schema-v3 narrative policy, starter, reports, catalog audits, ImageGen review contract, publication tooling, novels, and WonderSwan projects share this repository with the typed desktop UI. The signed app embeds only a minimal hash-verified framework payload. |
...
| Yokoi hardware utilities | Yokoi Boot and Yokoi Cart Service at the immutable source revision recorded in [`SOURCE_PROVENANCE.md`](https://github.com/RegionallyFamous/SwanSong-Desktop/blob/main/SOURCE_PROVENANCE.md) | Separately executable GPLv3 WonderSwan programs used by Cartridge Tools. Their payloads, license, notice, and corresponding-source location are verified independently from the GPLv2 Mac executable. |
...
| First-party homebrew releases | Identified immutable source trees, including [`StoryForge/games`](https://github.com/RegionallyFamous/SwanSong-Desktop/tree/main/StoryForge/games) in this repository | Licensed ROM source and exact-tag release assets referenced by the signed catalog. A catalog entry cannot replace the source tree's own license and provenance. |
...
hardware, backup, restore, and bootstrap boundaries.
