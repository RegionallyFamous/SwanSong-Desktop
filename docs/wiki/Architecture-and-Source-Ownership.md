...
| First-party homebrew releases | Identified immutable source trees, including [`StoryForge/games`](../../StoryForge/games) in this repository | Licensed ROM source and exact-tag release assets referenced by the signed catalog. A catalog entry cannot replace the source tree's own license and provenance. |
