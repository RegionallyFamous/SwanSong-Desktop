...
release with the built-in SwanSong Open IPL `open-bootstrap-v3` Color identity,
complete CPU/background history, and memory filtered only to the Sound-DMA
initiator. The manifest is bound to 7,283,585 cycles, 15 completed frames, the
exact ROM and Open IPL identities, zeroed initial IRAM, and the exact
